...
async function checkServer() {
  ...
  try {
    ...
    // kreach / kosha 둘 다 정상일 때만 '정상'
    const ok = d.services?.kreach === '정상' && d.services?.kosha === '정상';
    txt.textContent = ok ? '서버 정상 연결됨 — K-REACH · KOSHA 준비됨' : `서버 연결됨 (일부 서비스 점검 중)`;
    if (!ok) bar.classList.add('warn');
  } catch {
    bar.classList.add('warn');
    txt.textContent = '서버 연결 실패 — Flask(localhost:5000) 실행 여부 확인';
  }
}
